...
class TradingDecisionEngine:
    ...
    def analyze(self, data):
        ...
        log.info(f"Contexte reçu: signal={signal}, adx={adx}, adx_rising={adx_rising}, macd={macd_state}, pivot={last_pivot}, bars={bars_since}, vol={volatility}, impulse={impulse}")

        # ── 1. DÉTECTION DE RANGE ──
        # Si ADX < 20 et ne monte pas → marché en range → pas d'entrée
        if adx < 20 and not adx_rising:
            return False, "range_detected", 0, "ADX faible et plat — marché en range, pas d'entrée"

        # ── 2. DÉTECTION FIN DE MOVE ──
        # Si ADX > 40 et commence à baisser → tendance épuisée
        if adx > 40 and not adx_rising:
            return False, "end_of_move", 0, f"ADX={adx:.0f} en baisse — fin de move probable"

        # ── 3. PIVOT TROP VIEUX ──
        # Si le pivot est trop loin → signal affaibli
        if bars_since > 10:
            return False, "pivot_too_old", 0, f"Pivot trop ancien ({bars_since} bougies) — signal affaibli"

        # ── 4. VOLATILITÉ INSUFFISANTE ──
        if volatility != "high":
            return False, "low_volatility", 0, "Volatilité insuffisante — pas d'entrée"

        # ── 5. COHÉRENCE DU SIGNAL ──
        if signal == "long":
            if macd_state != "bullish":
                return False, "macd_incoherent", 0, "MACD incohérent avec signal long"
            if last_pivot != "low":
                return False, "pivot_incoherent", 0, "Pivot incohérent avec signal long"
            if impulse != "bullish":
                return False, "impulse_incoherent", 0, "Impulsion incohérente avec signal long"

        elif signal == "short":
            if macd_state != "bearish":
                return False, "macd_incoherent", 0, "MACD incohérent avec signal short"
            if last_pivot != "high":
                return False, "pivot_incoherent", 0, "Pivot incohérent avec signal short"
            if impulse != "bearish":
                return False, "impulse_incoherent", 0, "Impulsion incohérente avec signal short"

        # ── 6. SCORE DE CONFIANCE ──
        score = 0
        if adx > 25: score += 20
        if adx > 30: score += 10
        if adx_rising: score += 20
        if bars_since <= 3: score += 20
        elif bars_since <= 5: score += 10
        if volatility == "high": score += 15
        if impulse in ["bullish", "bearish"]: score += 15
